use super::{Buffer, F, diff_factor_u16 as diff_factor};
...
pub fn bilateral_filter<const C: usize, const C_PLUS_1: usize>(
    ...
    let inv_sigma_range = 1.0 / (sigma_range * upper_bound as F);
    // TODO this is way too big
    let range_table_f = std::array::from_fn::<F, { u16::MAX as usize + 1 }, _>(|i| {
    ...
    for i in 0..w * h {
        ...
        for c in 0..C {
            dst_i[c] = ((up_i[c] + dp_i[c]) * fac).clamp(0., u16::MAX as F) as u16;
        }
    }
}
